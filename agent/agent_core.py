...
def _enforce_intent_db_coverage(
    ...
    needs_duck_temporal = ("review" in q or "registered" in q) and any(yr in q for yr in _YEARS)
...
class AgentCore:
    ...
    async def run(self, request: QueryRequest, query_executor=None) -> AgentResponse:
        ...
        if mongo_sq and duck_sq and join_direction == "duckdb_first":
            ...
            meta_sq = next(
                (sq for sq in sub_queries if sq.database_type == "github_repos_metadata"),
                None,
            )
            art_sq = next(
                (sq for sq in sub_queries if sq.database_type == "github_repos_artifacts"),
                None,
            )
            if meta_sq and art_sq:
                ...
                if repo_names:
                    ...
                    nrepos = len(repo_names)
                    merge_operations.append(
                        f"github_repos_metadata->artifacts join on {nrepos} repos"
                    )
            ...
            if pg_sq and sql_sq:
                ...
                if pg_ids:
                    try:
                        ...
                        sqlite_schema = self.ctx.get_schema_for_db("sqlite", self._active_dataset)
                        new_sql_query = self._generate_sqlite_with_ids(
                            request.question, sqlite_schema, purchase_ids_sql
                        )
                        sql_sq = SubQuery(
...
- Return only the SQL query, no explanation, no markdown code fences"""
    ...
    def _synthesize(self, question: str, raw_results: dict, dataset: str = "") -> str:
        ...
        prompt = self.prompts.synthesize_response(
            question, enriched, {}, dataset=self._active_dataset
        )
